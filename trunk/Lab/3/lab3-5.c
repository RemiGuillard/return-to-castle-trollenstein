...
void init(void)
{
  ...
  progsky = loadShaders("sky-5.vert", "sky-5.frag");
  progtex = loadShaders("tex-5.vert", "tex-5.frag");
  ...
  LoadTGATextureSimple("grass.tga", &teaTex);
...
void	drawMyObject2t(Model *object, GLuint tex1, GLuint tex2, GLuint prog, GLfloat* mat) {
  ...
  if (object->texCoordArray != NULL) {
    ...
    glUniform1i(glGetUniformLocation(prog, "texUnit1"), 0); // Texunit 0 
    glUniform1i(glGetUniformLocation(prog, "texUnit2"), 1); // Texunit 1
  }
  DrawModel(object);
...
void	drawMyObject(Model *object, GLuint texture, GLuint prog, GLfloat* mat) {
  ...
  if (object->texCoordArray != NULL) {
    ...
    glUniform1i(glGetUniformLocation(prog, "texUnit"), 0); // Texunit 0 
  }
  DrawModel(object);
  glBindTexture(GL_TEXTURE_2D, 0);
}
...
void display(void)
{
  ...
  teapotMatrix[0] = cos(t); teapotMatrix[2] = sin(t);
  teapotMatrix[8] = -sin(t); teapotMatrix[10] = cos(t);
  drawMyObject2t(teapot, teaTex, dirtTex, progtex, teapotMatrix);
  ...
  teapotMatrix[0] = cos(t); teapotMatrix[2] = sin(t);
  teapotMatrix[8] = -sin(t); teapotMatrix[10] = cos(t);
  drawMyObject2t(teapot, flowerTex, skyTex, progtex, teapotMatrix);
